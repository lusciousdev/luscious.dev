...
class InfoForm(forms.Form):
  username = LastFmUsernameField(label="Username", max_length=128, widget=forms.TextInput(attrs={'class': 'char-field'}))
  period   = forms.ChoiceField(choices = PERIOD_CHOICES, label = "Period", widget=forms.Select(attrs={'class': 'choice-field'}))
  size     = forms.ChoiceField(choices = SIZE_CHOICES, label = "Size", widget=forms.Select(attrs={'class': 'choice-field'}), initial = '3')