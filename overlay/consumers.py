...
class OverlayConsumer(TwitchConsumer):
    ...
    def owner_or_editor(self):
        ...
        try:
            ...
            editormatch = self.overlay.owner.editor_set.get(
                id_type=0, identifier=twitchaccount.uid
            )
            return True
        ...
        for email_address in self.user.emailaddress_set.filter(verified=True).all():
            try:
                editormatch = self.overlay.owner.editor_set.get(
                    id_type=1, identifier=email_address.email
                )
                return True
            except:
                pass
        ...
        try:
            editormatch = self.overlay.owner.editor_set.get(
                id_type=2, identifier=self.overlay_user.identifier
            )
            return True
        except:
            pass
    ...
    def connect(self):
        ...
        try:
            self.twitch_account = self.overlay.owner.socialaccount_set.get(
                provider="twitch"
            )
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if len(self.response_list) == 1:
            self.send_command(
                self.response_list[0]["command"], self.response_list[0]["data"]
            )
        ...
        self.queue_broadcast(
            {
                ...
                "data": {
                    "uid": (
                        None
                        if self.overlay_user is None
                        else self.overlay_user.identifier
                    ),
    ...
    def request_refresh(self, data: dict):
        if self.owner_or_editor():
            self.queue_broadcast({"command": "refresh", "loopback": True, "data": {}})
    ...
    def send_chat_message(self, data: dict):
        ...
        messageObj = ChatMessage.objects.create(
            overlay=self.overlay, user=self.user, message=data["message"]
        )
    ...
    def get_chat_history(self):
        ...
        for chat_message in self.overlay.chatmessage_set.order_by("-timestamp").all()[
            :250
        ]:
            message_dict = {
                "username": chat_message.user.username,
                "uid": OverlayUser.objects.get(
    ...
    def trigger_item_event(self, data: dict):
        if "event" not in data:
            self.queue_command(
                "error",
                "Could not trigger an event because you did not provide an event.",
            )
            return
        ...
        if (
            item_model.get_simple_type() == "horse_game"
            and data["event"] == "reset_item"
        ):
    ...
    def record_canvas_event(self, data: dict):
        ...
        if item_model != CanvasItem:
            self.queue_command(
                "error",
                f"Can't trigger a canvas event on a {item_model.get_pretty_type()}, silly.",
            )
            return
        ...
        if data["event"] == "start_action":
            ...
            actionObj = CanvasAction.objects.create(
                canvas=item_instance,
                user=self.user,
                action=action,
                action_data=action_data,
            )
        elif data["event"] == "add_points":
            actionObj: CanvasAction = (
                self.user.canvasaction_set.filter(canvas=item_instance, user=self.user)
                .order_by("-timestamp")
                .first()
            )
    ...
    def broadcast_events(self, event):
        ...
        overlay_user_id = (
            None if self.overlay_user is None else self.overlay_user.identifier
        )
        event_list = list(
            filter(
                lambda command: command.get("loopback", False)
                or (command.get("data", {}).get("uid", "") != overlay_user_id),
                event_list,
            )
        )
