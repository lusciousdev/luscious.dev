...
var g_TwitchUID = ("twitchuid" in demoScriptData) ? demoScriptData.twitchuid : undefined;
...
function connectWebsocket()
{
  ...
  if (window.location.protocol == "https:")
    protocol = "wss:"
  g_Websocket = new WebSocket("{0}//{1}/ws/twitch/".format(protocol, window.location.host));
...
window.addEventListener('load', function(windowLoadEvent) {
  ...
  $("#predict-game").click((e) => {
    sendWebsocketMessage("start_prediction", { "duration": 60, "title": "Who will win?", "outcomes": g_HorseGame.activeRacers.map((v, i) => v.name) });
  });
