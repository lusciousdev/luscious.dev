...
function renderCanvas(t = undefined)
{
  ...
  case "rectangle":
    if (g_RectangleInProgress && g_MousePosition)
    {
      ...
      ctx.strokeRect(g_RectangleInProgress.points[0][0], g_RectangleInProgress.points[0][1], w, h);
    }
    break;
  case "polygon":
    if (g_PolygonInProgress)
    {
      ...
      {
        ctx.beginPath();
        ctx.moveTo(g_PolygonInProgress.points[0][0], g_PolygonInProgress.points[0][1]);
        g_PolygonInProgress.points.forEach((p) => {
          ctx.lineTo(p[0], p[1]);
        });
        if (g_MousePosition) ctx.lineTo(g_MousePosition[0], g_MousePosition[1]);
        ctx.lineTo(g_PolygonInProgress.points[0][0], g_PolygonInProgress.points[0][1]);
        ctx.stroke();
      }
    }
    break;
...
window.addEventListener('load', function(windowLoadEvent) {
  ...
  $("#canvas-container").mousemove((e) => {
    ...
    var x = Math.min(canvas.width(), Math.max(0, Math.floor(e.originalEvent.pageX - canvas.offset().left)));
    var y = Math.min(canvas.height(), Math.max(0, Math.floor(e.originalEvent.pageY - canvas.offset().top)));
    g_MousePosition = [x, y];
    ...
    switch (shapeType)
    {
      ...
      case "rectangle":
        if (g_RectangleInProgress)
        {
          ...
          g_Shapes.push({ type: "polygon", points: [p1, [p1[0], y], [x, y], [x, p1[1]]] })
